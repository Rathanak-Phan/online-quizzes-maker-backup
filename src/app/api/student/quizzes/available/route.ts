...
export async function GET(req: NextRequest) {
  try {
    ...
    const classes = await Class.find({ students: user.id }).select("_id quizzes");
    ...
    }).select("title description timeLimit totalPoints category createdAt assignedToClasses");
    ...
    const enriched = quizzes.map(q => {
      ...
      const inClasses = q.assignedToClasses?.filter(c => classIds.includes(c)) || [];
      return {
        ...q.toObject(),
